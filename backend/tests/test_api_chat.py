...
async def test_chat_endpoint_success():
    ...
    mock_response = {"messages": [AIMessage(content="I found 2 photos of money for you.")]}

    with patch("src.main.agent_app.ainvoke", new_callable=AsyncMock) as mock_ainvoke:
        mock_ainvoke.return_value = mock_response

        chat_request = {"message": "Find photos about money", "thread_id": "test_thread_123"}
...
async def test_chat_endpoint_persistence():
    ...
    with patch("src.main.agent_app.ainvoke", new_callable=AsyncMock) as mock_ainvoke:
        ...
        response = client.post("/api/chat/", json={"message": "Hello", "thread_id": "thread_abc"})
