...
def create_photo_record(db: Session, hash_str: str, file_path: str) -> Photo:
    photo = Photo(hash=hash_str, file_path=file_path)
    db.add(photo)
    db.commit()
    db.refresh(photo)
    return photo

def get_or_create_photo(db: Session, filepath: str):
