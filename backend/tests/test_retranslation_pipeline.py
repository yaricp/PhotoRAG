...
class TestStartRetranslation:
    ...
    async def test_skips_entirely_when_language_is_english(self):
        ...
        with (
            patch("src.retranslation_pipeline.init_pipeline_tasks") as mock_init,
    ...
    async def test_skips_entirely_when_no_photos(self):
        ...
        with (
            patch("src.retranslation_pipeline.init_pipeline_tasks") as mock_init,
            patch(
                "src.retranslation_pipeline.SessionLocal",
    ...
    async def test_creates_pipeline_task_per_photo(self):
        ...
        with (
            ...
            patch(
                "src.retranslation_pipeline.translate_description_task_for_retranslation", side_effect=fake_translate
            ),
        ):
    ...
    async def test_passes_correct_language_to_translate_task(self):
        ...
        with (
            patch("src.retranslation_pipeline.init_pipeline_tasks"),
            patch("src.retranslation_pipeline.SessionLocal") as mock_sl,
            patch(
                "src.retranslation_pipeline.translate_description_task_for_retranslation", side_effect=fake_translate
            ),
...
class TestTranslateDescriptionTaskForRetranslation:
    ...
    async def test_saves_translation_to_db(self):
        from src.tasks.translation_tasks import (
            translate_description_task_for_retranslation,
        )

        with (
            patch("src.tasks.translation_tasks._get_description_sync", return_value="A cat"),
            patch("src.tasks.translation_tasks._save_translation_sync") as mock_save,
            patch("src.tasks.translation_tasks.call_translation_model", new_callable=AsyncMock, return_value="Un gato"),
    ...
    async def test_uses_retranslation_phase_name(self):
        ...
        with (
            patch("src.tasks.translation_tasks._get_description_sync", return_value="Hello"),
            patch("src.tasks.translation_tasks._save_translation_sync"),
            patch("src.tasks.translation_tasks.call_translation_model", new_callable=AsyncMock, return_value="Hola"),
            patch("src.tasks.translation_tasks.track_task") as mock_ctx,
        ):
            mock_ctx.return_value.__aenter__ = AsyncMock(return_value=None)
            mock_ctx.return_value.__aexit__ = AsyncMock(return_value=False)
    ...
    async def test_skips_photo_with_no_description(self):
        ...
        with (
            patch("src.tasks.translation_tasks._get_description_sync", return_value=None),
            patch("src.tasks.translation_tasks.call_translation_model", new_callable=AsyncMock) as mock_translate,
            patch("src.tasks.translation_tasks.track_task") as mock_ctx,
        ):
            mock_ctx.return_value.__aenter__ = AsyncMock(return_value=None)
            mock_ctx.return_value.__aexit__ = AsyncMock(return_value=False)
...
class TestRetranslateAllEndpoint:
    def test_returns_202_and_started_status(self):
        ...
        with (
            patch("src.main.get_setting", return_value="es"),
    ...
    def test_does_not_start_pipeline_when_language_is_english(self):
        ...
        with (
            patch("src.main.get_setting", return_value="en"),
            patch("src.retranslation_pipeline.start_retranslation", new_callable=AsyncMock) as mock_start,
        ):
            client = TestClient(app)
            resp = client.post(
                "/api/translation/retranslate-all",
