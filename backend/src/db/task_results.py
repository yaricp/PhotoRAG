...
def save_result(task_id: str, result: str) -> None:
    ...
    conn = sqlite3.connect(RESULTS_DB)
    conn.execute("INSERT OR REPLACE INTO results (task_id, result) VALUES (?, ?)", (task_id, result))
    conn.commit()
    conn.close()
...
def save_error(task_id: str, error: str) -> None:
    ...
    conn = sqlite3.connect(RESULTS_DB)
    conn.execute("INSERT OR REPLACE INTO results (task_id, result) VALUES (?, ?)", (task_id, payload))
    conn.commit()
    conn.close()
...
def get_result(task_id: str) -> str | None:
    ...
    """
    conn = sqlite3.connect(RESULTS_DB)
    row = conn.execute("SELECT result FROM results WHERE task_id = ?", (task_id,)).fetchone()
    conn.close()
...
def cleanup_old_results(max_age_seconds: float = 3600) -> int:
    ...
    conn = sqlite3.connect(RESULTS_DB)
    cursor = conn.execute(
        "DELETE FROM results WHERE created_at < datetime('now', ? || ' seconds')", (f"-{int(max_age_seconds)}",)
    )
