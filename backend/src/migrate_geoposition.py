...
def run():
    with engine.connect() as conn:
        ...
        cols = [row[1] for row in conn.execute(__import__("sqlalchemy").text("PRAGMA table_info(photos)"))]
        ...
        conn.execute(__import__("sqlalchemy").text("ALTER TABLE geopositions DROP COLUMN photo_id"))
