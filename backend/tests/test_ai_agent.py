...
def test_agent_tool_calling_logic(mock_llm):
    ...
    tool_call = ToolCall(name="search_photos_semantic", args={"query": "cat", "k": 5}, id="call_1")
    ...
    with patch("src.graphs.tools.get_photos_by_vector") as mock_service, patch("src.graphs.tools.SessionLocal"):
...
def test_agent_history_retention(mock_llm):
    ...
    mock_llm.invoke.side_effect = [AIMessage(content="Response 1"), AIMessage(content="Response 2")]
