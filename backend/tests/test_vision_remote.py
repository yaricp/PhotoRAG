    ...
    def test_passes_base64_image_in_message(self, tmp_path):
        ...
        with patch("src.model_services._build_langchain_vision_model", return_value=mock_llm):
            ...
            asyncio.get_event_loop().run_until_complete(
                _call_remote_vision(cfg, str(f), "describe")
            )
...
class TestCallVisionModelDispatch:
    ...
    async def test_local_mode_submits_huey_task(self):
        ...
        with patch("src.model_services.read_model_config_from_db",
             ...
             patch("src.model_services._wait_result", new_callable=AsyncMock,
                   return_value=json.dumps({"text": "a cat"})):
    ...
    async def test_remote_mode_calls_remote_vision(self):
        ...
        with patch("src.model_services.read_model_config_from_db", return_value=cfg), \
             patch("src.model_services._call_remote_vision",
                   new_callable=AsyncMock, return_value="beach") as mock_remote:
            result = await call_vision_model("/fake/path.jpg", "describe_scene")
        assert result == "beach"
