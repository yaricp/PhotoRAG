...
def test_task_creates_duplicate_record_for_close_photos(db):
    ...
    with (
        ...
        patch.object(task_mod, "find_perceptual_duplicates", return_value=[{"photo_id": 20, "hash_distance": 5}]),
...
def test_folder_scanner_dispatches_perceptual_hash_via_start_pipeline(db):
    ...
    with (
        patch.object(fs_mod, "check_photo_hash_exists", return_value=None),
