...
def run():
    with engine.connect() as conn:
        cols = [row[1] for row in conn.execute(sqlalchemy.text("PRAGMA table_info(photos)"))]
        ...
        conn.execute(sqlalchemy.text("ALTER TABLE photos ADD COLUMN is_archived BOOLEAN NOT NULL DEFAULT 0"))
        conn.commit()
